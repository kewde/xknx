...
Spec text (verbatim from spec):
    ...
    sequenceDiagram
        ...
        C->>S: A_FunctionPropertyCommand-PDU (Server IA, File Server OI, PID_FILE_COMMAND, data = Retrieve File)
        S->>C: A_FunctionPropertyState_Response-PDU (Client IA, File Server OI, PID_FILE_COMMAND, Return Code)
        alt the Return Code is "Command Successful", a file exists and will be transferred to the client
            loop until the whole file is transmitted
                Note right of S: The Management Server (File Server) shall transmit the file in consecutive A_FileStream_InfoReport-PDUs. Each APDU shall have an APDU-length of 14 or longer as given in PID_MAX_APDU_LENGTh. The last APDU may have a shorter length.
                S->>C: A_FileStream_InfoReport-PDU (Client IA, file handle, file_block_sequence_number, file_block)
            end
            Note right of S: mark end of file. The field file_block is not present.
            S->>C: A_FileStream_InfoReport-PDU (Client IA, file handle, file_block_sequence_number)
