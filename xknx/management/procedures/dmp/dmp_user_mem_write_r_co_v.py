"""
DMP_UserMemWrite_RCoV — KNX 03.05.02 §3.19.3 (PDF p. 109).
...
Spec text (verbatim from spec):
    ...
    sequenceDiagram
        ...
        opt Verify Mode is not active
            ...
            S->>C: A_PropertyValue_Response-PDU (object_index = DeviceObject, PID = PID_DEVICE_CONTROL, start_index = 01h, element_count = 01h, data = DD, ..)
            Note right of S: A_Disconnect.ind ⇒ error, if no data received ⇒ error
            C->>S: A_PropertyValue_Write-PDU (object_index = DeviceObject, PID = PID_DEVICE_CONTROL, start_index = 01h, element_count = 01h, data = DD, ..)
...
async def dmp_user_mem_write_r_co_v(xknx: XKNX) -> None:
    """DMP_UserMemWrite_RCoV — see module docstring for the verbatim spec text."""
    raise NotImplementedError(
        "DMP_UserMemWrite_RCoV (KNX 03.05.02 §3.19.3) — implementation pending"
    )
