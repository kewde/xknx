...
Spec text (verbatim from spec):
    ...
    sequenceDiagram
        ...
        Note over C,S: start_index = 1
        Note over C,S: repeat
        C->>S: A_Link_Read-PDU (group_object_number = GO.number, start_index = )
        S->>C: A_Link_Response-PDU (group_object_number = GO.number, sending_addresss = GO.sendingindex, start_address = , group_address_list = group_address_set)
        Note over C,S: The Management Server adds the received group_address_set to the GO.GAList.
        Note over C,S: until a negative response is returned (start_index = 0) or less than
    ```
