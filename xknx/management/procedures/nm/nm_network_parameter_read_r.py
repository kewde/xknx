...
Spec text (verbatim from spec):
    ...
    sequenceDiagram
        ...
        C->>S: A_NetworkParameter_Read-PDU (hop_count_type, parameter_type, priority, test_info)
        S->>C: A_NetworkParameter_Response-PDU (ASAP, hop_count_type, individual_address, parameter_type, priority, test_info, test_result)
        Note right of S: if the parameter_type is supported and the test is positive, the Management Server shall respond
        Note over C,S: …
        S->>C: A_NetworkParameter_Response-PDU (ASAP, hop_count_type, individual_address, parameter_type, priority, test_info, test_result)
        Note right of S: According the specification of the parameter_type in [05], the Management Server may send 1 or multiple responses.
    ```
