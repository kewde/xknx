...
Spec text (verbatim from spec):
    ...
    sequenceDiagram
        ...
        C->>S: A_NetworkParameter_Read-PDU (object_type = 1, PID = PID_TABLE, range = rangechecked, start_address = GAChecked)
        S->>C: A_NetworkParameter_Response-PDU (object_type = 1, PID = PID_TABLE, range = rangechecked, start_address = GAChecked)
        Note right of S: If Group Addresses are supported and the investigated Group Address is linked to one or more Group Objects, one single response is sent by the device.
    ```
