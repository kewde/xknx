"""
NM_TunnellingFeature_Set — KNX 03.05.02 §6.7 (PDF p. 189).
...
Spec text (verbatim from spec):
    ...
    sequenceDiagram
        ...
        C->>S: TUNNELLING_FEATURE_SET (Feature Identifier, Feature Value)
        ...
        Note over C: UDP: If the Tunnelling Client does not receive a TUNNELLING_ACK within the TUNNELLING_REQUEST_TIME_OUT (1 s) then it shall repeat the TUNNELLING_FEATURE_SET 1 time.
        Note over S: The Tunnelling Server shall within 3 s respond with a TUNNELLING_FEATURE_RESPONSE, repeating the Feature Identifier and including the Return Code and the Feature Value (also in case of a negative Return Code).
...
async def nm_tunnelling_feature_set(xknx: XKNX) -> None:
    """NM_TunnellingFeature_Set — see module docstring for the verbatim spec text."""
    raise NotImplementedError(
        "NM_TunnellingFeature_Set (KNX 03.05.02 §6.7) — implementation pending"
    )
