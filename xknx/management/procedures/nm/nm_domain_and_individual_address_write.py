...
Spec text (verbatim from spec):
    ...
    sequenceDiagram
        ...
        alt negative A_Connect.Lcon ⇒ IA_new is not occupied; continue with 2.
            ...
            S->>C: A_Disconnect-PDU (destination_address = IA_new)
            Note over C,S: If an A_Disconnect-PDU is received then IA_new shall be regarded as occupied; end procedure.
            alt no A_Disconnect-PDU is received
                ...
                Note over C,S: The Management Client shall accept any value of descriptor_type, also values ≠ 0.
        ...
        Note over C,S: 2. wait until Programming Mode is active in the device:
        Note over C,S: repeat until one A_IndividualAddress_Response-PDU is received
        C->>S: A_DomainAddress_Read-PDU ()
        S->>C: A_DomainAddress_Response-PDU (source_address = IA_current, domain_addres= DoA_current)
        Note right of S: one or more responses may be received from different devices
        Note right of S: time-out: 1 s
        Note over C,S: …
        Note over C,S: if more than one response is received ⇒ than Programming Mode is active in more than one device
        Note over C,S: end repeat
        Note over C,S: 3.set Domain Address and Individual Address
        opt Domain Address DoA_current != DoA_new
            C->>S: A_DomainAddress_Write-PDU (domain_address = DoA_new)
        end
        opt IA_current != IA_new
            C->>S: A_IndividualAddress_Write-PDU (new_address = IA_new)
        end
        Note over C,S: 4. verify and inactivate programming mode
        C->>S: A_Connect-PDU (destination_address = IA_new)
        C->>S: A_DeviceDescriptor_Read-PDU (descriptor_type = 0000h)
        S->>C: A_DeviceDescriptor_Response-PDU (descriptor_type, device_descriptor)
        C->>S: A_Restart-PDU ()
        Note over C,S: Abort the connection of the Management Client side Transport Layer.
    ```
