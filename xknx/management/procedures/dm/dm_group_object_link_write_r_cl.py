...
Spec text (verbatim from spec):
    ...
    sequenceDiagram
        ...
        C->>S: A_Link_Write-PDU (group_object_number = GO.number, flags.d = GO.action, flags.s = GO.sending, group_address = GO.value)
        Note right of S: The Management Server (device) shall add or delete the GA to or from the list of GAs assigned to the referred GO and set the sending flag, both according the flags.
        Note right of S: The Management Server shall reply with the list of GAs assigned to the referred GO (1); if it has a problem, it shall respond with a negative A_Link_Response-PDU.
        S->>C: A_Link_Response-PDU (group_object_number = GO.number, sending_addresss = GO.sendingindex, start_index = 1, group_address_list = GO.GAList)
    ```
