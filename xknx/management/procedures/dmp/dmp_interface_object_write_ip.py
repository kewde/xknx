...
Spec text (verbatim from spec):
    ...
    sequenceDiagram
        ...
        Note over C,S: The Management Client writes the Property Value with a DEVICE_CONFIGURATION_REQUEST.
        loop For each data block until all data are transferred
            C->>S: DEVICE_CONFIGURATION_REQUEST (CommunicationChannel ID = dmp_CommChannelID, cEMI-Frame = (M_PropWrite.req(IOT = mpp_Obj.Type, OI = mpp.Obj.Inst, PID = mpp_Prop.ID, NoE = mpp_Obj.NoE, SIx = mpp_Obj.SIx)
            ...
            S->>C: DEVICE_CONFIGURATION_REQUEST (CommunicationChannel ID = dmp_CommChannelID, cEMI-Frame = M_PropWrite.con(IOT = mpp_Obj.Type, OI = mpp_Obj.Inst, PID = mpp_Prop.ID, NoE = mpp_Obj.NoE, SIx = mpp_Obj.SIx, mpp_ErrorCode = Error Code))
