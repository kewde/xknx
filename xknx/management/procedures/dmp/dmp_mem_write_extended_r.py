"""
DMP_MemWrite_Extended_R — KNX 03.05.02 §3.22 (PDF p. 113).
...
Spec text (verbatim from spec):
    ...
    sequenceDiagram
        ...
        loop for each datablock (data size ≤ maximal size), until all data are transmitted
            C->>S: A_MemoryExtended_Write-PDU (Addr, Length, Data)
            S->>C: A_MemoryExtended_Write_Response-PDU (Return Code, Addr)
...
async def dmp_mem_write_extended_r(xknx: XKNX) -> None:
    """DMP_MemWrite_Extended_R — see module docstring for the verbatim spec text."""
    raise NotImplementedError(
        "DMP_MemWrite_Extended_R (KNX 03.05.02 §3.22) — implementation pending"
    )
