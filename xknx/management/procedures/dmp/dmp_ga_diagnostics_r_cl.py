...
Spec text (verbatim from spec):
    ...
    sequenceDiagram
        ...
        Note over C,S: NOTE 21 The GO Diagnostic commands that address Group Addresses do not require Diagnostic Mode to be enabled. The MaC may set this.
        Note over C,S: The MaS may now access Group Object Diagnostics.
        C->>S: A_FunctionPropertyCommand-PDU (object_index = Group Object Table.index 19), PID = PID_GO_DIAGNOSTICS, data = dmp_command + dmp_flags + dmp_group_address+ dmp_command_data)
        Note right of S: If the MaS supports Group Object Diagnostics in the addressed Interface Object, then it shall respond.
        S->>C: A_FunctionPropertyState_Response-PDU (object_index = Group Object Table.index PID = PID_GO_DIAGNOSTICS, data = dmp_return_code + dmp_command)
        Note over C,S: Depending on the specific command, the MaS or the user may further have to do the following.
        Note over C,S: 19) In an E-Mode MaS, the Property shall not be addressed in the Group Object Table but in the E-Mode Channel Object or Adjusted E-Mode Channel Object in which the GO resides that shall be diagnosed.
    ```
